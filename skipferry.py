...
class App(_BaseTk):
    ...
    def _build_ui(self):
        ...
        frm_log_body.pack(fill="both", expand=True, padx=6, pady=6)
        # 長いパスでも末尾まで見えるよう、縦・横両方のスクロールバーを出す
        self.txt_log = tk.Text(frm_log_body, height=12, wrap="none")
        lsb = ttk.Scrollbar(frm_log_body, orient="vertical", command=self.txt_log.yview)
        lhsb = ttk.Scrollbar(frm_log_body, orient="horizontal", command=self.txt_log.xview)
        self.txt_log.config(yscrollcommand=lsb.set, xscrollcommand=lhsb.set)
        self.txt_log.grid(row=0, column=0, sticky="nsew")
        lsb.grid(row=0, column=1, sticky="ns")
        lhsb.grid(row=1, column=0, sticky="ew")
        frm_log_body.rowconfigure(0, weight=1)
        frm_log_body.columnconfigure(0, weight=1)
